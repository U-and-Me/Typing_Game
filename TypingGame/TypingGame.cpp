...
void wordScan() {
	...
	gotoxy(80, 2); cout << "게임 시간 : " << 25 - user_time;
...
void GameTime() {
	...
	if (user_time > 25){
		...
		if (level == 1) {
			...
			system("cls");
			level++; // 2단계
			user_time = 0; startTime = 0; endTime = 0; word_speed = 6000; remove_speed = 5000;
			for (int i = 0; i < wordCount; i++)
				wordc[i] = 0;
			Play();
			...
			system("cls");
			level++; // 3단계
			user_time = 0; startTime = 0; endTime = 0; word_speed = 4000; remove_speed = 2000;
			for (int i = 0; i < wordCount; i++)
				wordc[i] = 0;
			Play();
		}
